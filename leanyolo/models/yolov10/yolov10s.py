...
- Eval mode: returns decoded detections per image as List[List[Tensor]], where
  ...
  (letterboxed) size. Thresholds controlled via attributes:
...
class YOLOv10s(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # In eval, return both branches for decode parity; training uses head's native dict
        if self.training:
            return self.head((p3, p4, p5))
        # Eval: compute both branches but return one2many to preserve API; cache both
    ...
    def decode_forward(self, raw: List[torch.Tensor] | dict):
        ...
        """
        # Official NMS-free decode (top-k) for parity
        strides = (8, 16, 32)
